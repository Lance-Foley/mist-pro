...
Rails.application.routes.draw do
  get("job_overview/index")

  namespace(:api) do
    namespace(:v1) do
      resources(:crew_members, only: %i[create destroy])
      resources(:job_assignments, only: %i[index show create update destroy])
      resources(:employee_job_assignments, only: %i[index show create update destroy])
    end
  end
